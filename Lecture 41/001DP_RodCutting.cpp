...
int f(int n, const vector<int>& p) {
	...
	// deicide the size of next cut

	int maxSoFar = 0;

	for (int j = 1; j <= n; j++) {

		// make a cut of size j, and then ask your friend
		// to find the maximum profit we can get by partitioning
		// the remaining rod of length n - j
